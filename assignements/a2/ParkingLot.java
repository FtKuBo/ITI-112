...
public class ParkingLot {
	...
	/**
	 * Attempts to park a car in the lot. Parking is successful if a suitable
	 * parking spot
	 * is available in the lot. If some suitable spot is found (anywhere in the
	 * lot), the car
	 * is parked at that spot with the indicated timestamp and the method returns
	 * "true".
	 * If no suitable spot is found, no parking action is taken and the method
	 * simply returns
	 * "false"
	 * 
	 * @param c         is the car to be parked
	 * @param timestamp is the simulation time at which parking is attempted for car
	 *                  c
	 * @return true if c is successfully parked somwhere in the lot, and false
	 *         otherwise
	 */
	...
	public int getTotalCapacity() {
		...
		for (int zab = 0; zab < numRows; zab++)
			for (int j = 0; j < numSpotsPerRow; j++)
				if (lotDesign[zab][j] != CarType.NA)
					count++;
	...
	public int getTotalOccupancy() {
		...
		for (int i = 0; i < numRows; i++)
			for (int zok = 0; zok < numSpotsPerRow; zok++)
				if (occupancy[i][zok] != null)
					count++;
