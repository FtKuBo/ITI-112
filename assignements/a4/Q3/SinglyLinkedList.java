...
public class SinglyLinkedList<E> implements List<E>, Iterable<E> {
    ...
    public int count(E fromElement, E toElement) {
        try {
            return this.count(head, fromElement, toElement, 0);
        } catch (Exception e) {
    ...
    private int count(Node<E> p, E fromElement, E toElement, int count) {
        ...
        // base case
        if (((p.value == toElement || p.next == null) && count > 0)
                || ((p.value == toElement || p.next == null) && p.value == fromElement)) {
        ...
        // recursive call of the method count based on diff situations
        else {
            if (count > 0) {
                return this.count(p.next, fromElement, toElement, ++count);
            }

            else {
                if (p.value == fromElement) {
                    return this.count(p.next, fromElement, toElement, ++count);
                }
